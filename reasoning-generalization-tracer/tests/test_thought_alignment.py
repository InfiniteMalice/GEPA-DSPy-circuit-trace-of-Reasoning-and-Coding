...
def test_honest_uncertainty_aligns():
    ...
    aligned, s_match, s_epi = classify_thought_alignment(trace, "I don't know", None)
    assert aligned
    assert s_match >= 0.8
    assert s_epi >= 0.5
...
def test_confident_wrong_reasoning_can_align():
    trace = "Therefore the sum yields 9 after consistent steps."  # incorrect vs expected 8
    aligned, s_match, s_epi = classify_thought_alignment(trace, "9", {"expected_answer": 8})
    assert aligned
    assert s_match >= 0.8
    assert s_epi >= 0.5
